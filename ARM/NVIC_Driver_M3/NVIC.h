...
/**
 * @def 	GET_IRQ_PRIORITY
 * @brief 	GETs Group and Subgroup from Irq no.
 */
#define GET_IRQ_PRIORITY(X, Y) (((X) << (6)) | (((Y) & (0x3)) << (4)))
/**
 * @def 	ENABLE_ALL_INT
 * @brief 	Enable the Interrupts for all peripherals inside the NVIC
 */
#define ENABLE_ALL_INT   0xFFFFFFFF
/**
 * @def 	DISABLE_ALL_INT
 * @brief 	disable the Interrupts for all peripherals inside the NVIC
 */
#define DISABLE_ALL_INT  0x00000000
 ...
 * @param   IRQn Variable of uint8 , Interrupt Request Number is number assigned to peripheral as it is offest of them in vector table array ex (WWDG , TIM2)
 * @return  OK | NOT_OK
 */
 ...
 * @param   IRQn Variable of uint8 , Interrupt Request Number is number assigned to peripheral as it is offest of them in vector table array ex (WWDG , TIM2)
 * @param   Val  pointer  of uint8 , Pending Flag which to be read assigned to pointer
 * @return  OK | NOT_OK
 */
 ...
 * @param   IRQn Variable of uint8 , Interrupt Request Number is number assigned to peripheral as it is offest of them in vector table array ex (WWDG , TIM2)
 * @param   Val  pointer  of uint8 , Active Flag which to be read assigned to pointer
 * @return  OK | NOT_OK
 */
 ...
 * @param   priority_grouping Variable of uint32 , Priority Type (ALL_PREEMPTION , THREE_PREEMPTION_ONE_SUBGROUP)
 * @return  OK | NOT_OK
 */
 ...
 * @param   IRQn        Variable of uint8 , Interrupt Request Number is number assigned to peripheral as it is offest of them in vector table array ex (WWDG , TIM2)
 * @param   priority    Variable of uint8 , Priority (0..16)
 * @return  OK | NOT_OK
 */
uint8 DNVIC_SetPriority (uint8 IRQn, uint8 group, uint8 subgroup);

/**
 * @brief   Function to Read peripheral Priority 
 * @param   IRQn        Variable of uint8 , Interrupt Request Number is number assigned to peripheral as it is offest of them in vector table array ex (WWDG , TIM2)
 * @param   priority    Pointer to uint8 , Priority which to be read
 * @return  OK | NOT_OK
 */
 ...
 * @return  NA
 */
void DNVIC_voidSetBASEPRI(uint8 priority);
/**
 * @brief   Function to change vector Table offest  
 * @param   offset Variable of uint32 , Change vector Table offest  
 * @return  NA
 */
